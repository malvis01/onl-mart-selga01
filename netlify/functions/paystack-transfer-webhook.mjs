...
export default async req=>{
 ...
 if(["transfer.success","transfer.failed","transfer.reversed"].includes(event.event)){
   ...
   const {data:order}=await supabase.from("orders").select("id,delivery_fee").eq("delivery_fee_transfer_reference",ref).maybeSingle();
   if(order){
     await supabase.from("orders").update({delivery_fee_payout_status:status,delivery_fee_released:status==="success"?amount:0,updated_at:new Date().toISOString()}).eq("id",order.id);
     await supabase.from("transactions").update({delivery_fee_payout_status:status}).eq("delivery_fee_transfer_reference",ref);
   }
 }
