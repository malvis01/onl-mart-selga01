...
async function removeOrder(order){
  const id = order.id;
  // Transactions own commission records, so remove commissions by transaction id first.
...
export default async function handler(){
  ...
  for(const order of orders || []){
    const terminalTime = new Date(order.updated_at || order.completed_at || order.cancelled_at || order.created_at).getTime();
