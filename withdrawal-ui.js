(function(){
  ...
  async function refreshSellerWithdrawal(){try{const d=await realSellerBalance();const box=document.getElementById("sellerWithdrawalInfo");if(!box)return;const bal=typeof money==="function"?money(d.available_balance):"₦"+Number(d.available_balance||0).toLocaleString("en-NG",{minimumFractionDigits:2});box.innerHTML=`<div class="record-line"><span>Available balance</span><strong>${bal}</strong></div><div class="record-line"><span>Bank</span><strong>${String(d.business?.bank_name||"")}</strong></div><div class="record-line"><span>Account number</span><strong>${String(d.business?.account_number||"")}</strong></div><div class="record-line"><span>Withdrawal PIN</span><strong>${d.pin_set?"Set":"Not set"}</strong></div>`;}catch(e){}}
